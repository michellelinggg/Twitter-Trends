...
def extract_words(text):
    ...
    """
    words0, words1, empty_space=list(text), [], [""]
    for letter in words0:
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    average, total, count = make_sentiment(None), 0, 0
    word_list=tweet_words(tweet)
    for word in word_list:
        if has_sentiment(get_word_sentiment(word)):
            total=total+sentiment_value(get_word_sentiment(word))
            count+=1
    if not count==0:
...
def find_centroid(polygon):
    ...
    CY = 0
    if A == 0:
        return (polygon[0][0], polygon[0][1], A)
    for i in range(len(polygon)-1):
...
def find_center(polygons):
    ...
    for poly in (polygons):
        CxA = CxA + (find_centroid(poly)[0] * find_centroid(poly)[2])
        CyA = CyA + (find_centroid(poly)[1] * find_centroid(poly)[2])
        A = A + find_centroid(poly)[2]
    CX = CxA/A
    CY = CyA/A
    return (CX, CY)
...
def find_closest_state(tweet, state_centers):
    ...
    for k in state_centers:
        center = state_centers[k]
        new = geo_distance(tweet_loc, center)
        if new<old:
            state = k
            old= new
    return state
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        if state in tweets_by_state:
            ...
            tweets_by_state[state] = [tweet] #tweets_by_state[state] = (tweet,)
...
def most_talkative_state(term):
    ...
    state, num_tweets=None, 0
    for item in state_dict:
        if len(state_dict[item])>num_tweets:
            state, num_tweets=item, len(state_dict[item])
    return state
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        for elem in tweets_by_state[state]:
            if has_sentiment(analyze_tweet_sentiment(elem)) or analyze_tweet_sentiment(elem)==0:
                total=total+sentiment_value(analyze_tweet_sentiment(elem))
                count+=1
